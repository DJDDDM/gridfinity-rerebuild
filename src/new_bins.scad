...
module gridfinity_bin()
{
    difference()
    {
        lip() attach(BOT, TOP) upper() attach(BOT, TOP) middle() attach(BOT, TOP) bottom();
        clearance();
    }
}
...
module middle()
{
    ...
    module first_part()
    {
        color_this("grey")
            rect_tube(h = bin.height_unit, size = standard.length, isize1 = standard.length - 2 * bin.wall_thickness,
                      isize2 = standard.length - 2 * bin.wall_thickness, rounding = standard.outer_rounding,
                      irounding = 0) children();
    }
}
    ...
    module first_part()
    {
        color_this("blue")
            rect_tube(h = upper.height1, size = standard.length, isize1 = standard.length - 2 * upper.width1,
                      isize2 = standard.length - 2 * lip.width3, rounding = standard.outer_rounding, irounding = 0)
                children();
    }
    ...
    module second_part()
    {
        color_this("green")
            rect_tube(h = upper.height2, size = standard.length, isize1 = standard.length - 2 * upper.width2,
                      isize2 = standard.length - 2 * lip.width1, rounding = standard.outer_rounding, irounding = 0)
                children();
    }
    ...
    module third_part()
    {
        color_this("red")
            rect_tube(h = upper.height3, size = standard.length, isize1 = standard.length - 2 * upper.width3,
                      isize2 = standard.length - 2 * upper.width2, rounding = standard.outer_rounding, irounding = 0)
                children();
    }
}
    ...
    module first_part()
    {
        color_this("purple")
            rect_tube(h = lip.height1, size = standard.length, isize1 = standard.length - 2 * lip.width1,
                      isize2 = standard.length - 2 * math.epsilon, rounding = standard.outer_rounding, irounding = 0,
                      anchor = TOP) children();
    }
    ...
    module third_part()
    {
        color_this("firebrick")
            rect_tube(h = lip.height3, size = standard.length, isize1 = standard.length - 2 * lip.width3,
                      isize2 = standard.length - 2 * lip.width2, rounding = standard.outer_rounding, irounding = 0)
                children();
    }
}